/**
 * Authentication token detection utilities
 * 
 * Auto-detect tokens from supported registered providers.
 */

import { existsSync, readFileSync } from 'node:fs';
import { homedir, platform } from 'node:os';
import { join } from 'node:path';
import type { ProviderTokens } from '../types.js';
...
/**
 * Extract provider tokens from OpenCode auth.
 * Returns a tokens object suitable for passing to getUsage().
 */
function getTokensFromOpenCodeAuth(): ProviderTokens {
 ...
 * Auto-detect tokens from supported registered providers.
 * 
 * @returns Detected tokens object
 */
export function detectAuthTokens(): ProviderTokens {
  return getTokensFromOpenCodeAuth();
}
